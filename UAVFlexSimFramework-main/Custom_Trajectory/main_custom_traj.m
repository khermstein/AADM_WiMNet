...
threshold_distance = 0.0020; % 250m
...
%% Calculate the waypoints and orders
...
% Equality constraint (none for now)
...
% Extract node visiting order - safe to assume first edge is first to
% travel as it must start at 1
...
ordered_nodes(1) = [];
disp(ordered_nodes);
% lat_stops = lat_samples(ordered_nodes);
% lon_stops = lon_samples(ordered_nodes);
% disp("Ordered stops are");
% disp(lat_stops);
% disp(lon_stops);

%Waypoint for traveling to and from LW3
waypoint_lon = -78.69421;
...
while ii <= length(lat_stops)
    if any(ordered_nodes(ord_i) == N3_idcs)
...
for ii = 1:length(lat_stops) % While there are base stations left
    global uav_speed
    current_position = 0;
    ...
    min_distance = pdist2([lat_drone, lon_drone], [lat_stops(ii), lon_stops(ii)]);
    % Get the coordinates of the next base station
    next_lat = lat_stops(ii);
    next_lon = lon_stops(ii);
